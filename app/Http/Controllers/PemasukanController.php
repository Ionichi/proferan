...
class PemasukanController extends BaseController
{
    ...
    {
        $data = Transaksi::where('user_id', Auth::user()->id)->where('type', 'pemasukan')->where(function($q) use($request) {
    ...
    public function createUpdate(Request $request)
    {
        ...
        if(empty($request->id)) {
            $result = $this->moduleService->processModuleCreate('pemasukan', $request);
            $response = $result->getData();
        } else {
            $result = $this->moduleService->processModuleUpdate('pemasukan', $request);
            $response = $result->getData();
        }
    ...
    {
        try {
            $transaksi = Transaksi::where('id', $id)->where('user_id', Auth::user()->id)->where('type', 'pemasukan')->firstOrFail();
    ...
    {
        $result = $this->moduleService->processModuleDestroy('pemasukan', $request);
        $response = $result->getData();
