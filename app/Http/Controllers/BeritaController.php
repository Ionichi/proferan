...
class BeritaController extends BaseController
{
    ...
    public function table() {
        ...
        return DataTables::of($data)
            ...
            ->editColumn('gambar', function($data) {
                return '<img src="'. asset('storage/berita/'. $data->gambar). '" width="100px" height="100px">';
            })
    ...
    public function storeOrUpdate(Request $request) {
        ...
        if($request->hasFile('gambar')) {
            if(File::exists('storage/berita/'. $storeOrUpdate->gambar)) {
                File::delete('storage/berita/'. $storeOrUpdate->gambar);
            }
            $file = $request->file('gambar');
            $extension = $file->getClientOriginalExtension();
            $filename = time() . '-' . Str::slug($request->judul, '-') . '.'. $extension;
            $file->move('storage/berita/', $filename);
    ...
    public function hapus(Request $request) {
        ...
        if(File::exists('storage/berita/'. $data->gambar)) {
            File::delete('storage/berita/'. $data->gambar);
        }
        $data->delete();
